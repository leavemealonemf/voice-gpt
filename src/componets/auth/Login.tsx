...
const Login: FC<Props> = ({setCurrentModuleHandler}) => {
    ...
    return (
        <form className='w-96 flex flex-col' onSubmit={onSubmit}>
            <span className="text-white font-bold text-3xl mb-5">{t('sign_in')}</span>
            <div className="mb-6">
            ...
            <button type="submit" className="text-white self-start bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">{t('to_come_in')}</button>
            <span
                onClick={() => setCurrentModuleHandler(currentModuleTypes.REGISTER)} 
                className='text-white text-xs mt-3 font-normal cursor-pointer'>{t('not_registered')} <u>{t('register_button')}</u>
            </span>
